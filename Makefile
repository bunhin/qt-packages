...
rpm-src-control:
	mkdir -p redhat ;
	for file in packaging/redhat/m4/* ; do m4 -D "PACKAGE_NAME=$(PACKAGE_NAME)" -D "PRODUCT_VERSION=$(PRODUCT_VERSION)" -D "PACKAGE_VERSION=$(PACKAGE_VERSION)" -D "RELEASE_NUMBER=$(RELEASE_NUMBER)" -D "BINARY=0" -D "CLIENT=1" -D "SERVER=0" -D "PREFIX=$(PREFIX)" < "$$file" > redhat/"`basename "$$file"`" ; done ;
